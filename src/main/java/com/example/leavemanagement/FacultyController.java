...
public class QuestionController {
    ...
    public String assignleave(Model model , HttpSession session){
        ...
        String sql = "select * from leave_master where leave_id in (select leave_id from leave_faculty_assign where name ='"+ email +"')";
        List<Map<String,Object>> leave_master=jdbc.queryForList(sql);
        model.addAttribute("leave_master",leave_master);
        return "applyleave";
    }

    @GetMapping("/f_profile")
    public String f_profile(HttpSession session , Model model) {
        String email=(String)session.getAttribute("email");
